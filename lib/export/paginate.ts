export function paginateContent(htmlString: string): HTMLDivElement[] {
  ...
  children.forEach((child) => {
    ...
    const content = currentPage.querySelector<HTMLDivElement>(".page-content");
    if (!content) return;
    ...
    if (currentHeight + height > pageHeight) {
      ...
      const newContent =
        currentPage.querySelector<HTMLDivElement>(".page-content");
